...
The iframe declares `allow="autoplay"`. User activation from a parent-page click does not propagate into a cross-origin frame, and the opaque sandbox origin is cross-origin by definition, so without this permissions-policy delegation Chrome keeps the editor's `AudioContext` suspended after Play. Strudel itself only initializes audio on a `mousedown` inside its own document, and the pinned superdough never calls `resume()` there; the host bridge therefore resumes the context and runs that initialization when it receives `evaluate`, then reports the resulting `AudioContext` state to the parent as an `audioState` event (after each evaluation, with a grace period while a suspended context may still be waiting for the output device, and on every later `statechange`). If the context stays suspended, the host shows an "Enable sound" button inside the iframe; that click is a user gesture of the sandboxed document and resumes the context in browsers that do not honor the delegation. The delegation grants no DOM, storage, or network access.
